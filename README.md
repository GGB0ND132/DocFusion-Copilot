...
- **Prompt 工程**：15 个 few-shot 示例 + 匹配原则 + 验证步骤
...
```bash
cp .env.example .env
# 编辑 .env，填写 DOCFUSION_OPENAI_API_KEY
...
| 对话持久化 | 4 | CRUD 生命周期、AgentService 自动持久化、删除不存在对话安全性、批量创建列举 |
| 追溯 | 1 | 事实 → 文档 → 模板使用链路 |
| Benchmark 报告 | 1 | 错误分类 + Markdown 报告生成 |
| Prompt 工程 | 2 | Few-shot 摘要意图规划、Few-shot QA 意图规划 |
| LLM-First QA | 2 | 摘要无预过滤验证、QA 无预过滤验证 |
| 解析器 | 11 | Markdown / Text / XLSX / DOCX / PDF 解析器完整性 |
...
系统支持 12 种自然语言意图。在 Agent 页面的输入框中直接输入即可：

| 功能 | 示例提示词 | 说明 |
|---|---|---|
| **智能填表** | `帮我智能填表`、`用这些数据回填模板` | 需先上传模板文件（xlsx/docx），再发送指令 |
| **查询事实** | `查询上海的GDP`、`列出北京人口数据` | 按实体/字段精确查询事实库 |
| **提取事实** | `从这些报告提取各城市的GDP和人口` | 从文档中抽取新事实入库 |
| **文档摘要** | `总结一下这些文档`、`给我一个概述` | 基于事实和文档块生成 Markdown 摘要 |
| **通用问答** | `山东省空气质量怎么样？`、`COVID-19确诊人数有多少` | 基于事实库的开放问答 |
| **文档编辑** | `把甲方替换为乙方`、`帮我把用户要求.txt的内容删除` | 支持查找替换和内容删除，编辑后提供下载 |
| **格式整理** | `帮我整理一下格式`、`统一标题样式` | 对 docx/md/txt 执行排版规范化 |
| **字段提取** | `提取所有城市的GDP和人均收入字段` | 按指定字段导出结构化 JSON 表格 |
| **数据导出** | `导出为Excel`、`把提取结果保存为xlsx` | 将事实库导出为 JSON 或 XLSX 文件 |
| **状态查询** | `当前系统有多少文档？`、`查询上传进度` | 查看系统文档/事实统计 |
| **事实追溯** | `追溯 fact_001 的来源` | 查看事实的来源文档、段落和证据文本 |
| **闲聊** | `你好`、`你能做什么？` | 寒暄或功能说明 |

### 典型工作流

1. **上传文档**：在「工作台」页面上传 docx/md/txt/xlsx/pdf 文件，等待解析完成（绿色状态点）
2. **查看数据**：在工作台右侧查看提取的事实和 JSON 数据
3. **模板回填**：切换到「Agent」页面 → 点击📎上传模板文件 → 输入 `帮我智能填表` → 右侧查看回填结果并下载
4. **问答与分析**：直接在 Agent 输入框提问，如 `总结一下山东省空气环境质量`
5. **编辑文档**：输入 `把所有"甲方"替换为"委托方"` → 点击下载按钮获取编辑后的文件
6. **批量管理**：在工作台使用复选框选择多个文档，点击批量删除按钮

### 提示词技巧

- **越具体越好**：`查询山东省2024年PM2.5月均值` 比 `查一下数据` 效果更好
- **指定实体和字段**：`提取上海、北京的GDP和常住人口` 比 `提取一些数据` 更精确
- **编辑说清楚**：`把第三页的"甲方"替换为"委托方"` 比 `帮我改一下` 更准确
- **模板回填加条件**：`将日期从2020/7/1到2020/8/31的数据填入模板`
